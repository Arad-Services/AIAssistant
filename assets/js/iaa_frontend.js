        ...
        jQuery('*').click(function (e) {
            if (iaa_selectionMode) {
                if (jQuery(this).children().length == 0 || jQuery(this).is('a') || jQuery(this).is('button') || jQuery(this).is('img') || jQuery(this).is('select')) {
                    ...
                    window.parent.iaa_confirmSelectElement(this);
...
function iaa_initAssistant() {
    if (sessionStorage.iaa_closed != 1) {
        ...
        if (iaa_data.settings.avatarType == 1) {
            ...
            jQuery('.iaa_avatarContainer').append('<img class="iaa_avatar_mouth" onload="iaa_repositionMouth();" style="left:' + iaa_data.settings.avatarWidth / 2 + 'px; width: auto; margin-left: -' + iaa_data.settings.avatarMouthWidth / 2 + 'px;" src="' + iaa_data.settings.avatarTalkImg + '" />');
...
function iaa_say(content) {
    ...
    setTimeout(function () {
        ...
        if (content.text || content.interactions) {
            ...
            if (content.text.indexOf('[interaction') > -1) {
                var interactionID = content.text.substr(content.text.indexOf('[interaction') + 13, content.text.indexOf(']', content.text.indexOf('[interaction')));
                interactionID.replace(' ', '');
                interactionID = interactionID.substr(interactionID.indexOf("'") + 1, interactionID.lastIndexOf("'") - (interactionID.indexOf("'") + 1));

                var interaction = iaa_getBackupInteraction(0, interactionID);
                var starttxt = content.text.substr(0,content.text.indexOf('[interaction'));
                var endTxt = content.text.substr(content.text.indexOf(']',content.text.indexOf('[interaction'))+1,content.text.length);
                contentText = starttxt+interaction.value+endTxt;
               // contentText = content.text.replace(content.text.substr(content.text.indexOf('[interaction'), content.text.indexOf(']', content.text.indexOf('[interaction') + 1)), interaction.value) + content.text.substr(content.text.indexOf(']', content.text.indexOf('[interaction')) + 1, content.text.length);
            ...
            jQuery.each(content.interactions, function () {
                ...
                switch (options.type) {
                    case "button":
                        buttons += '<a href="javascript:" class="iaa_btn" onclick="iaa_stepBtnClick(this);" data-id="' + options.itemID + '">' + options.label + '</a>';
                        break;
                    case "textfield":
...
function iaa_initActions(step) {
    ...
    jQuery.each(step.content.actions, function () {
        ...
        switch (action.type) {
            case "sendEmail":
                //if (sessionStorage.iaa_step_done == 0) {
                jQuery.ajax({
            ...
            case "changeUrl":
                if (!sessionStorage.iaa_redir || sessionStorage.iaa_redir != step.id) {
                    ...
                    if (currentUrl != action.url && action.url + '/' != currentUrl && currentUrl + '/' != action.url) {
                       ...
                       // if (jQuery.urlParam('iaa_action') && jQuery.urlParam('iaa_action') == 'preview') {
