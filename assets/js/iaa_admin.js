...
if (jQuery('#iaa_winActivation').is('[data-show="true"]') && document.location.href.indexOf('admin.php?page=iaa_')>-1){
   ...
   /* jQuery('.colorpick').each(function () {
    ...
    //jQuery('#iaa_settings_avatarImg').on('input',iaa_autoSaveSettings);
...
function iaa_loadSteps() {
    ...
    jQuery.ajax({
        ...
        success: function (rep) {
            ...
            iaa_steps = rep.steps;
            /*jQuery('#iaa_stepsContainer').css({
                height: iaa_params.previewHeight + 'px'
            });*/
...
function iaa_openSettings() {
    ...
    /*if (iaa_settings.avatarWidth > 0) {
     jQuery('#iaa_avatarCustomPreview').css({
     width: iaa_settings.avatarWidth,
     height:  iaa_settings.avatarHeight
     });
     }*/
    jQuery('#iaa_avatarCustomPreview_mouth').css({
        left: '50%',
...
function iaa_addLinkInteraction(data) {
    ...
    $item.append($col2);
    $item.append('<div class="col-md-1"><a href="javascript:" class="iaa_conditionDelBtn" onclick="iaa_conditionRemove(this);"><span class="glyphicon glyphicon-remove"></span></a> </div>');
    if (data) {
...
function iaa_actionSave() {
    ...
    if (jQuery('#iaa_actionBubble').data('type')) {
        type = jQuery('#iaa_actionBubble').data('type');
        ...
        if (chkCreate) {
            ...
            if (jQuery('#iaa_actionBubble').attr('data-item') && jQuery('#iaa_actionBubble').attr('data-item') != "") {                
...
function iaa_actionDel() {
    ...
    if (itemID && itemID != "") {
        var index = jQuery('#' + itemID).index();
        iaa_currentStep.actions.splice(index, 1);
...
function iaa_startSelectElement() {
    ...
    jQuery('body').append('<iframe id="iaa_websiteFrame" src="' + iaa_data.websiteUrl + '" onload="iaa_websiteFrameLoaded();"></iframe>');
    ...
    jQuery('.iaa_selectElementPanel').append('<p><a href="javascript:" class="btn btn-primary" onclick="iaa_selectionSelectElement();">' + iaa_data.txt_selectBtn + '</a>&nbsp;&nbsp;<a href="javascript:" class="btn btn-danger" onclick="iaa_stopSelectElement();">' + iaa_data.txt_cancel + '</a></p>');
    jQuery('#iaa_loader').fadeIn();
...
function iaa_selectionSelectElement() {
    ...
    jQuery('.iaa_selectElementPanel').append('<p><a href="javascript:" class="btn btn-danger" onclick="iaa_stopSelectElement();">' + iaa_data.txt_cancel + '</a></p>');
...
function iaa_confirmSelectElement(el) {
    ...
    jQuery('.iaa_selectElementPanel').append('<p><a href="javascript:" class="btn btn-primary" onclick="iaa_confirmedSelectElement();">' + iaa_data.txt_yes + '</a>&nbsp;&nbsp;<a href="javascript:" onclick="iaa_startSelectElement();" class="btn btn-danger">' + iaa_data.txt_no + '</a></p>');
}
...
function iaa_previewStep() {
    ...
    jQuery.each(iaa_currentStep.interactions, function () {
        var options = this;
        lastElementID = options.elementID;
        switch (options.type) {
            case "button":
